...
## 6.1 Roadmap + Current Gaps (What’s Missing for Functionality)\n+\n+**Roadmap:** See `docs/roadmap.md` for a phased plan and milestones.\n+\n+**Missing to reach a functional MVP:**\n+- Real **screen capture + detection** for the Accept button (pixel sampling and/or OCR).\n+- Real **input automation** (pyautogui/AutoIT) to click Accept and start/stop queue.\n+- **Calibration** flow to capture Accept button coordinates.\n+- **Persistence + validation** for configuration and auth token.\n+- **Android app MVP** (UI, WebSocket client, notifications, and controls).\n+\n+---\n+\n ## 7) Example Workflow
## 7) Example Workflow
